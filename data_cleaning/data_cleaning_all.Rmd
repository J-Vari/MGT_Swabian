...
combined_tasks_male %>%
  ...
  select(-`UTC Timestamp`, -`UTC Date and Time`, -`Object Name`)%>%
...
origin_authen_female%>%
  ...
  filter(!(Response_corrected == "__other" & is.na(Response_Text_identity_lang.var)))%>%
  distinct(`Participant Public ID`, Audio, Target_Concept, Variety, .keep_all = TRUE)%>%
  select(-Response, -`Object ID`,-`UTC Date and Time`, -`UTC Timestamp`)->origin_authen_female
```
### Prep origin & authenticity join
...
origin_authen_male_vs_female%>%
  ...
  filter(!(Response_corrected == "__other" & is.na(Response_Text_identity_lang.var)))%>%
  distinct(`Participant Public ID`, Audio, Target_Concept, Variety, .keep_all = TRUE)%>%
  select(-Response, -`Object ID`,-`UTC Date and Time`, -`UTC Timestamp`)->origin_authen_male_vs_female
```
# Prep origin & authenticity join
...
```{r questionnaire2-lang_pop_standardise}
...
combined_questionnaire%>%
  distinct_all()->combined_questionnaire
...
as.factor(combined_questionnaire$`Participant Public ID`)-> combined_questionnaire$`Participant Public ID`
...
data_tasks%>%
left_join (combined_questionnaire, by= "Participant Public ID")-> data_all

data_all %>%
  distinct(`Participant Public ID`, `Audio_Track`, `Attribute`, .keep_all = TRUE)-> data_better
```
# Final data set
```{r check_final_data}

# Check if there are any matching rows
matching_rows <- data_tasks %>%
  semi_join(combined_questionnaire, by = c("Participant Public ID", "Task Name"))

# Display the number of matching rows
nrow(matching_rows) # 0 rows matching

unmatched_rows <- data_tasks %>%
  anti_join(combined_questionnaire, by = c("Participant Public ID", "Task Name")) # all rows umatched
...
data_better%>%
  group_by(gender)%>%
  summarise(n_distinct(`Participant Public ID`))->pp_gender # works

data_better%>%
  ungroup()%>%
  summarise(mean(own_dialect), sd (own_dialect), min (own_dialect), max(own_dialect))->own_lang.variety # NAs

data_better%>%
  group_by(party)%>%
  summarise(n_distinct(`Participant Public ID`))->sum_party # works
