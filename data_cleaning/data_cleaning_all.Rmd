...
data_wide%>%
  pivot_longer(cols = 41:83, names_to = "variety_numeric", values_to = "lang.variety")%>%
...
```{r questionnaire2-lang_pop, eval=FALSE, include=FALSE}

## attempt to create freq table failed


##frequ lang.table separate
data_wide%>%
  select(`Participant Public ID`, 41:83)->lang.

lang.%>%
  rowwise()%>%
  summarise(frequency = n(), .groups = 'drop')->freq
  
group_by(`Participant Public ID`, `variety_numeric`, `lang.variety`) %>%
  summarise(frequency = n(), .groups = 'drop')->freq

## chat GPT frequency approach

# Calculate frequency of each unique language value for each participant
 data_long %>%
  group_by(`Participant Public ID`, `variety_numeric`, `lang.variety`) %>%rowwise()%>%
  summarise(frequency = n(), .groups = 'drop')->freq
...
data_all%>%
  group_by(`Participant Public ID`) %>%
  summarise(na_count_lang.expo = sum(is.na(lang.exposure_test)), na_count_own_dialect = sum(is.na(own_dialect)), na_count_party= sum(is.na(party)))->NA_quest
...
data_all%>%
  ...
  select(`Participant Public ID`, Audio_Track, Attribute, Response, gender, age, party, `UTC Timestamp.y`, lang.exposure_test)->pp_check
