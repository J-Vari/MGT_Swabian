...
questionnaire%>%
  select("Participant Public ID", starts_with("languages_caregiver1"), starts_with("languages_caregiver2"), "other_languages", "own_dialect", "party", "party-text", matches("^social_desirability_[A-I]-quantised$"), matches("^populism_[A-D]$"), starts_with("political_"))-> pp_lang_pop_wide
...
filter (`Participant Public ID` != "280475315358931")->combined_questionnaire
```
...
```
# combine questioonaire with task data
...
combined_tasks_male%>%
left_join (combined_questionnaire, by= `Participant Public ID`)-> combined_data_male
