...
questionnaire %>%
  mutate(filtered_dialects = sapply(other_languages, extract_dialects, dialects)) -> questionnaire

# clean up
questionnaire %>%
  separate(filtered_dialects, into = c("other_languages_1", "other_languages_2", "other_languages_3", "other_languages_4"), sep = ",", extra = "merge", fill = "right") %>%
...
#generalize answers and save them to other_languages
questionnaire %>%
  mutate(other_languages = case_when(
    rowSums(!is.na(select(., starts_with("other_languages_"))) & select(., starts_with("other_languages_")) == "Schwäbisch") > 0 ~ "Schwäbisch",
    rowSums(!is.na(select(., starts_with("other_languages_"))) & select(., starts_with("other_languages_")) != "Schwäbisch") > 0 ~ "Other dialect",
    TRUE ~ NA_character_
  )) -> questionnaire

questionnaire %>%
  select(-other_languages_1, -other_languages_2, -other_languages_3, -other_languages_4) -> questionnaire
